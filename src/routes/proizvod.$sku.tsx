...
function getSpecs(p: Product): { label: string; value: string }[] {
  ...
  const extra: { label: string; value: string }[] = {
    Burrovi:          [{ label: "Materijal", value: "Kaljeni čelik (Rockwell 62–65 HRC)" }, { label: "Kompatibilnost", value: "Mazzer, Anfim, Mythos" }],
    Analizatori:      [{ label: "Senzor", value: "SONY IMX-500 multispektar" }, { label: "Interfejs", value: "USB-C / Wi-Fi 6" }],
    "Mlinske stanice":[{ label: "Materijal kućišta", value: "7075-T6 CNC aluminijum" }, { label: "Masa", value: "≈ 4,2 kg" }],
    Pribor:           [{ label: "Prečnik", value: "58 / 58.5 mm" }, { label: "Materijal", value: "Nerđajući čelik / Aluminijum" }],
    Servis:           [{ label: "Metoda", value: "CNC precizno oštrenje" }, { label: "Rok isporuke", value: "3–5 radnih dana" }],
  }[p.category] ?? [];
  return [...common, ...extra];
}
...
function getMaintenance(p: Product): string {
  const map: Record<Product["category"], string> = {
    Burrovi:          "Nakon svakih 200 kg mlevene kafe preporučujemo proveru hrapavosti sečiva. Čišćenje suvom četkicom posle svake smene. Izbegavati vlagu i abrazivna sredstva.",
    Analizatori:      "Kalibrišite uređaj Agtron-EBC standardima svakih 30 dana. Sočiva čistite isključivo mikrofiber krpom. Čuvati na temperaturi 5–35 °C, bez direktne sunčeve svetlosti.",
    "Mlinske stanice":"Svake nedelje pažljivo obrišite kućište neabrazivnom krpom. Proveravajte zategnutost vijaka nosača burra na svakih 50 radnih sati.",
    Pribor:           "Perite toplom vodom i neutralnim sapunom. Ne koristiti u mašini za pranje sudova. Obrišite do suha odmah nakon pranja da izbegnete mrlje od vodenog kamenca.",
    Servis:           "Nakon servisa oštrenja preporučujemo 2–3 kg sezoniranja (\"seasoning\") pre prve ozbiljne upotrebe kako bi sečiva dostigla optimalne performanse.",
  };
    ...
    <PageShell>
        ...
        <div className="max-w-4xl mx-auto px-6 grid md:grid-cols-2 gap-12">
          ...
          <div className="space-y-6">
            ...
            {activeTab === "Opis" && (
              <div className="space-y-4 animate-reveal">
                ...
                <p className="text-muted-foreground text-sm leading-relaxed">
                  Ovaj proizvod je deo laboratorijskog asortimana EEF Online Professional — svaki artikal prolazi internu proveru pre isporuke i uključuje verifikacioni list iz naše optičke laboratorije.
                </p>
              </div>
            )}
            ...
            {activeTab === "Održavanje" && (
              <div className="space-y-4 animate-reveal">
                <h3 className="font-display text-2xl font-bold tracking-tight">Uputstvo za održavanje</h3>
                <p className="text-muted-foreground text-sm leading-relaxed">{getMaintenance(product)}</p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between border border-border p-4">
                <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Cena</span>
                <span className="font-display font-black text-2xl">{fmtRSD(product.price)}</span>
              </div>
              <div className="flex items-center justify-between border border-border p-4">
                <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Zaliha</span>
                <span className={`font-display font-bold text-sm ${product.stock > 0 ? "text-card-foreground" : "text-muted-foreground"}`}>
      ...
      {crossSell.length > 0 && (
        <section className="py-16 border-b border-border">
          <div className="max-w-4xl mx-auto px-6">
            <div className="mb-8">
              <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-accent mb-2">Preporuka</div>
              <h2 className="font-display text-2xl font-bold tracking-tight">Često se kupuje zajedno</h2>
            </div>
      ...
      <section className="py-24">
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <h2 className="font-display text-3xl font-bold tracking-tight mb-6">Potrebna vam je dodatna informacija?</h2>
  ...
  return (
    <div className="flex gap-4 border border-border bg-background p-4 hover:border-primary/50 transition-colors">
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-between gap-2">
        <div>
          <div className="font-mono text-[10px] uppercase tracking-widest text-accent mb-0.5">{p.category}</div>
          <Link to={`/proizvod/${p.sku}`} className="font-display font-bold text-sm leading-tight hover:text-primary transition-colors line-clamp-2">
            {p.name}
          </Link>
        </div>
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            onClick={() => { add(p.sku); setOpen(true); }}
