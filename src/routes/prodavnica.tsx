    ...
    <PageShell>
      ...
      <section className="border-b border-border bg-surface/30">
        <div className="max-w-7xl mx-auto px-6 py-6 flex flex-col md:flex-row gap-4 md:items-center justify-between">
          <div className="flex flex-wrap gap-2">
            {CATEGORIES.map((c) => (
              <button
                key={c}
                onClick={() => setCat(c)}
                className={`px-4 py-2 font-display text-[11px] uppercase tracking-widest border transition-colors ${
                  cat === c
          ...
          <input
            ...
            className="bg-background border border-border px-4 py-2 font-sans text-sm focus:outline-none focus:border-primary w-full md:w-72"
          />
        </div>
      </section>
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-6">
          {filtered.length === 0 ? (
            <p className="text-center text-muted-foreground py-24">Nema rezultata za zadatu pretragu.</p>
          ) : (
      ...
      <section className="border-t border-border bg-surface/40 py-12">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
          <div>
            <div className="font-mono text-primary text-xs tracking-widest mb-2">[ NAPOMENA ]</div>
          ...
          <Link
            to="/contact"
            className="border border-border px-6 py-3 font-display text-xs uppercase tracking-widest hover:border-primary hover:text-accent transition-colors"
          >
            Pošalji B2B upit
          </Link>
        </div>
  ...
  return (
    <article className="group border border-border bg-background hover:border-primary/60 transition-colors flex flex-col">
      <Link to={`/proizvod/${p.sku}`} className="relative aspect-[4/3] overflow-hidden bg-surface">
        ...
        />
        <div className="absolute top-3 left-3 font-mono text-[10px] uppercase tracking-widest bg-background/80 backdrop-blur px-2 py-1 text-accent">
          {p.category}
        </div>
      </Link>
      <div className="p-5 flex-1 flex flex-col gap-3">
        ...
        <div className="flex items-end justify-between pt-3 border-t border-border">
          <div>
            <div className="font-display font-black text-xl text-foreground">{fmtRSD(p.price)}</div>
          ...
          <button
            ...
            className="bg-foreground text-background font-display font-bold uppercase tracking-widest text-[10px] px-4 py-3 hover:bg-primary hover:text-primary-foreground transition-colors"
          >
            U korpu +
          </button>
        </div>
